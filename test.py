...
def main():
    ...
    Width_set = config["Width_set"] # Установить ширину разрешения
    Height_set = config["Height_set"]  # Установить высокое разрешение
    framerate_set = config["framerate_set"]  # Установить частоту кадров
    num = config["num"]  # Время частоты кадров получения (для целей отладки вы можете установить последующее получение изображения как цикл while для получения неограниченного цикла)
    ...
    if dev_num == 0:
        ...
        for i in range(num):
            ...
            img.save(r"image//" + str(i) + str("-") + mtime + ".jpg")  # Сохранить картинку локально
